  ...
  return (
    <div className="relative w-full h-[450px] md:h-[500px] rounded-2xl overflow-hidden shadow-2xl group my-6">
      ...
      <div className="absolute inset-0 flex items-center px-6 md:px-12">
        <div className="flex flex-col md:flex-row items-center gap-8 w-full max-w-[1200px] mx-auto">
          ...
          <div className="text-white flex-1 text-center md:text-left space-y-4">
            ...
            <div className="pt-4">
              <Link 
                to={`/movie/${movie.id}`}
                className="inline-block bg-red-600 hover:bg-red-700 text-white px-8 py-3 rounded-full font-bold transition-transform transform hover:scale-105 shadow-lg"
              >
                Xem Chi Tiết
              </Link>
            </div>
      ...
      <button onClick={prevSlide} className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/10 hover:bg-white/30 p-3 rounded-full text-white backdrop-blur-sm transition opacity-0 group-hover:opacity-100">
        <ChevronLeft size={30} />
      </button>

      <button onClick={nextSlide} className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/10 hover:bg-white/30 p-3 rounded-full text-white backdrop-blur-sm transition opacity-0 group-hover:opacity-100">
